...
//
//  This example file demonstrates how to take distance measurements as fast as possible, when you first plug-in
//  a LIDAR-Lite into an Arduino it runs 250 measurements per second (250Hz). Then if we setup the sensor by reducing
//  the aquisiton record count by 1/3 and incresing the i2c communication speed from 100kHz to 400kHz we get about
//  500 measurements per second (500Hz). Now if we throttle the reference and preamp stabilization processes during
//  the distance measurement process we can increase the number of measurements to about 750 per second (750Hz).
//
